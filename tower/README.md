...
then open http://localhost:7778. **First run: start at `/welcome`** — the
sectioned walkthrough, and the seed of the FAQ. Until your chain writes
its first receipt, the UI serves a loudly-bannered synthetic sample
(`demo/demo-chain.jsonl`, generated by `demo/make-demo-chain.mjs`, shaped
after the double-blind test behind "The Bones Do Remember") so the blank
page still teaches; it disappears the moment a real receipt exists.
