...
describe('L29: the log states which commit it describes', () => {
  ...
  it('the CLI stamps a scratch copy and reports current', (t) => {
    // Stamping tests operate on a copy: even SIGKILL cannot leave a fabricated
    // verification claim in the tracked disclosure file.
    const file = tmpFile(SAMPLE_LOG);
    t.after(() => fs.rmSync(path.dirname(file), { recursive: true, force: true }));
    const options = { encoding: 'utf8', env: { ...process.env, LOTOR_LIMITS_FILE: file } };
    execFileSync(process.execPath, ['bin/limits-pin.js', '--stamp'], options);
    const out = execFileSync(process.execPath, ['bin/limits-pin.js', '--check'], options);
    assert.match(out, /matches your checkout/);
  });

  it('the CLI catches a stale pin in a scratch copy (exit 1)', (t) => {
    const file = tmpFile(SAMPLE_LOG);
    t.after(() => fs.rmSync(path.dirname(file), { recursive: true, force: true }));
    writePin(file, { commit: OTHER_HASH, subject: 'some other tree', date: '2026-08-23' });
    assert.throws(() => execFileSync(process.execPath, ['bin/limits-pin.js', '--check'], {
      encoding: 'utf8', env: { ...process.env, LOTOR_LIMITS_FILE: file }
    }), (error) => {
      assert.strictEqual(error.status, 1);
      assert.match(error.stdout, /reading a description of somewhere else/i);
      assert.ok(error.stdout.includes(OTHER_HASH));
      return true;
    });
  });
});
